...
def download_all(cache: bool = True, skip_download: bool = False) -> dict:
    """
    ...
    """
    stocks_cache = CACHE_DIR / f"ohlcv_{CFG.universe.name}_{CFG.data.start_date}_{CFG.data.end_date}.parquet"
    index_cache = CACHE_DIR / f"index_{CFG.universe.benchmark_ticker}.parquet"

    if skip_download:
        if not (stocks_cache.exists() and index_cache.exists()):
            raise FileNotFoundError(
                f"Cache files not found. Cannot skip download. "
                f"Expected: {stocks_cache} and {index_cache}"
            )
        logger.info("Forced loading cached data: %s, %s", stocks_cache, index_cache)
        stocks_df = pd.read_parquet(stocks_cache)
