...
def validate_metrics_integrity(
    ...
    for name, value in metrics.items():
        ...
        # Skip roundness check for standard boundary values (0.0, 1.0, -1.0)
        # and for p-values which are highly likely to be 0 or 1 under extreme conditions.
        is_pvalue = "p_value" in name.lower() or "pvalue" in name.lower()
        if value != 0.0 and value != 1.0 and value != -1.0 and not is_pvalue:
...
def scan_for_hardcoded_patterns(file_content: str) -> List[Tuple[int, str]]:
    ...
    patterns = [
        ...
        # Direct assignment or dictionary mapping of known fabricated values
